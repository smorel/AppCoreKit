            ...
            /*else if([[mutableTarget objectForKey:key] isKindOfClass:[NSArray class]]){
                ...
                for(id object in [mutableTarget objectForKey:key]){
                    if([object isKindOfClass:[NSDictionary class]]){
                        NSMutableDictionary* result = [mutableTarget applyHierarchically:object toDictionary:object];
    ...
    for(id key in [self allKeys]){
        ...
        if([object isKindOfClass:[NSArray class]]){
            ...
            for(id subObject in object){
                if([subObject isKindOfClass:[NSDictionary class]]){
                    ...
                    if(![subObject isKindOfClass:[NSMutableDictionary class]]){
                        ...
                        [dico initAfterLoadingForObjectWithKey:@""];
                        [subObject release];
                    }else{
                        [dico initAfterLoadingForObjectWithKey:@""];
                    }
			...
			for(NSString* format in fromatGroups){
				...
				[dico setObject:[NSValue valueWithNonretainedObject:self] forKey:CKCascadingTreeParent];
                [dico initAfterLoadingForObjectWithKey:key];
			}
		}
		[object release];
	}
