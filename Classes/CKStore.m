...
- (NSArray *)fetchAttributesForItemNamed:(NSString *)itemName withNames:(NSArray *)attributesNames {
	return [self fetchAttributesForItemNamed:itemName withNames:attributesNames resultType:CKStoreAttributeResultType];
}

- (id)fetchAttributesForItemNamed:(NSString *)itemName withNames:(NSArray *)names resultType:(CKStoreResultType)resultType {
...
- (NSArray *)fetchItemsWithPredicate:(NSPredicate *)predicate {
	return [self.manager.objectContext fetchObjectsForEntityForName:@"CKItem" predicate:predicate sortedBy:nil limit:0];
}
