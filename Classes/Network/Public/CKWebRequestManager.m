...
@interface CKWebRequestManager ()<UIAlertViewDelegate> 
    ...
    if (self = [super init]) {
        ...
        self.disconnectBlock = ^{
            ...
            if(!bself.alertDisplayed){
                ...
                UIAlertView *alertView = [[[UIAlertView alloc] initWithTitle:_(@"No Internet Connection")
                                                                     message:_(@"You don't seems to have Internet access right now, do you want to try again?")
                                                                    delegate:self
                                                           cancelButtonTitle:_(@"Dismiss") otherButtonTitles:_(@"Retry"),nil]autorelease];
                
                bself.alertDisplayed = YES;
                [alertView show];
            }
        };
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex;{
    // the user clicked Dismiss
    if (buttonIndex == 0)
    {
        self.alertDisplayed = NO;
        [self didHandleDisconnect];
    }else{
        self.alertDisplayed = NO;
        [self didHandleDisconnect];
